...
module Conversions =
    ...
    let ``WithCalendar() Gregorian <-> Julian`` (data: YemodaAnd<Yemoda>) =
        ...
        let gdate = GregorianCalendar.Instance.GetCalendarDate(y, m, d).ToCalendarDay()
        let jdate = JulianCalendar.Instance.GetCalendarDate(julian.Year, julian.Month, julian.Day).ToCalendarDay()
