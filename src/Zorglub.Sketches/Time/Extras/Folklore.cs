...
namespace Zorglub.Time.Extras
{
    ...
    public static class Folklore // Friday the 13th (Gregorian calendar)
    {
        ...
        [Pure]
        public static bool IsUnluckyFriday(XCivilDate date) =>
            // On vérifie d'abord le jour de la semaine (propriété la plus rapide à obtenir).
            date.DayOfWeek == DayOfWeek.Friday
            && date.Day == 13;

        [Pure]
        public static bool IsUnluckyFriday(GregorianDate date) =>
            // On vérifie d'abord le jour de la semaine (propriété la plus rapide à obtenir).
            date.DayOfWeek == DayOfWeek.Friday
            && date.Day == 13;

        [Pure]
        public static IEnumerable<XCivilDate> FindUnluckyFridays(Range<XCivilDate> range)
        {
            var min = range.Min;
            var max = range.Max;
        ...
        [Pure]
        public static IEnumerable<GregorianDate> FindUnluckyFridays(Range<GregorianDate> range)
        {
            var min = range.Min;
            var max = range.Max;
