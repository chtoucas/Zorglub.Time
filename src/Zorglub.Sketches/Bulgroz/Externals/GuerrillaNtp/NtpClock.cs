...
namespace Zorglub.Bulgroz.Externals.GuerrillaNtp
{
    ...
    public record NtpClock(NtpResponse Response)
    {
        ...
        /// </remarks>
        public static NtpClock LocalFallback { get; } = InitLocalFallback();
        ...
        {
            get => 0.5 * (
                Response.ReceiveTimestamp - Response.OriginTimestamp
                - (Response.DestinationTimestamp - Response.TransmitTimestamp));
        }
        ...
        {
            get => Response.ReceiveTimestamp - Response.OriginTimestamp
                + (Response.DestinationTimestamp - Response.TransmitTimestamp);
        }

        private static NtpClock InitLocalFallback()
        {
            var time = DateTime.UtcNow;
            return new NtpClock(new NtpResponse
            {
