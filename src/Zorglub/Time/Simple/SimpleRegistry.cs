...
{
    /// <summary>Represents the minimum value for the ID of a user-defined calendar.
    /// <para>It is also the absolute minimum value for <see cref="MinId"/>.</para>
    /// <para>This field is a constant equal to 64.</para></summary>
    public const int MinMinId = (int)Cuid.MinUser;

    /// <summary>Represents the maximum value for the ID of a user-defined calendar.
    /// <para>It is also the absolute maximum value for <see cref="MaxId"/>.</para>
    /// <para>This field is a constant equal to 127.</para></summary>
    public const int MaxMaxId = (int)Cuid.Max;

    /// <summary>Represents the dictionary of (lazy) calendars, indexed by their keys.</summary>
    private readonly ConcurrentDictionary<string, Lazy<SimpleCalendar>> _calendarsByKey;

    private int _lastId;

    /// <summary>Initializes a new instance of the <see cref="SimpleRegistry"/> class.</summary>
    ...
    /// <summary>Initializes a new instance of the <see cref="SimpleRegistry"/> class.</summary>
    /// <exception cref="AoorException"><paramref name="minId"/> is not within the range
    /// [<see cref="MinMinId"/>..<see cref="MaxMaxId"/>].</exception>
    /// <exception cref="AoorException"><paramref name="maxId"/> is not within the range
    /// [<paramref name="minId"/>..<see cref="MaxMaxId"/>].</exception>
    ...
    /// <summary>Gets the minimum value for the ID of a user-defined calendar.</summary>
    public int MinId { get; }

    /// <summary>Gets the absolute maximum value for the ID of a calendar.</summary>
    public int MaxId { get; }

    /// <summary>Returns true if the registry is in its pristine state; otherwise returns false.
    /// </summary>
    public bool IsPristine => _lastId < MinId;

    /// <summary>Returns true if the registry is full; otherwise returns false.
    /// <para>Once this property becomes true, the registry enters a read-only state, i.e. any
    /// further attempt to add a new calendar will fail.</para></summary>
    public bool IsFull => _lastId >= MaxId;

    /// <summary>Determines whether "fail fast" is disabled or not.
    /// <para>The default behaviour is to enable fail fast.</para>
    /// <para><i>For testing purposes only</i>.</para></summary>
    internal bool DisableFailFast { get; set; }

    public Action<SimpleCalendar>? CalendarCreated { get; init; }

    /// <summary>Gets the list of keys of all fully constructed calendars at the time of the request.
    /// <para>This collection may also contain a few dirty keys, those paired with a calendar with
    /// an ID equal to <see cref="Cuid.Invalid"/>.</para></summary>
    internal ICollection<string> Keys => _calendarsByKey.Keys;

    /// <summary>Gets the maximum number of calendars that this instance may contain.</summary>
    public int MaxNumberOfCalendars => MaxId + 1; // <= 128 and >= 1

    /// <summary>Gets the maximum number of user-defined calendars that this instance may contain.
    /// </summary>
    public int MaxNumberOfUserCalendars => MaxId - MinId + 1; // <= 64 and >= 1

    /// <summary>Gets the number of system calendars.</summary>
    public int NumberOfSystemCalendars { get; private set; } // <= 64 and >= 0

    /// <summary>Gets the raw number of calendars, including <i>dirty</i> calendars.</summary>
    internal int RawCount => _calendarsByKey.Count;

    /// <summary>Counts the number of calendars.</summary>
    [Pure]
    public int CountCalendars() =>
        // Beware, we cannot use _calendarsByKey.Count because it also includes
        // the dirty calendars. The correct formula is given by:
    ...
    /// <summary>Counts the number of user-defined calendars.</summary>
    [Pure]
...
{
    /// <summary>Adds a calendar to the current instance.
    ...
    /// afterwards; only <see cref="RawCount"/> will continue to work properly.</para></summary>
    ...
    /// <summary>Pre-validates the parameters.</summary>
    ...
    /// <summary>Creates a new <see cref="SimpleCalendar"/> instance from the specified temporary
    /// calendar then call <see cref="CalendarCreated"/>.
    /// <para>Returns a calendar with ID <see cref="Cuid.Invalid"/> if the system already reached
    /// the maximum number of calendars it can handle.</para></summary>
    [Pure]
